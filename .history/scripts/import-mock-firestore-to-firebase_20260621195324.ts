...
function formatUnknown(e: unknown): string {
  try {
    if (e instanceof Error) return e.stack || e.message;
    return typeof e === "string"
      ? e
      : JSON.stringify(e, Object.getOwnPropertyNames(e as any), 2);
  } catch {
    return String(e);
  }
}
...
process.on("unhandledRejection", (reason) => {
  ...
  console.error("UnhandledRejection:", formatUnknown(reason));
});

process.on("uncaughtException", (err) => {
  // eslint-disable-next-line no-console
  console.error("UncaughtException:", formatUnknown(err));
  process.exit(1);
});
...
main()
  ...
  .catch((e: unknown) => {
    // eslint-disable-next-line no-console
    console.error("Seed script failed:", formatUnknown(e));
    process.exit(1);
  });

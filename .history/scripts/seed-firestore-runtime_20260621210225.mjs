...
async function seedCollection(firestore, collection, docs) {
  ...
  for (const item of docs) {
    const ref = firestore.collection(collection).doc(item.id);
    batch.set(ref, stripId(item), { merge: false });
    ops++;
...
async function main() {
  ...
  for (const [collection, docs] of Object.entries(collections)) {
    console.log(`Writing ${collection} (${docs.length})`);
    await seedCollection(firestore, collection, docs);
  }

  console.log("Firestore dummy data uploaded.");
}

...
function useFirestoreCollection<T>(collection: string, seedData?: T[]) {
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      setData(docs.map((d) => ({ ...d.data, id: d.id })));
      setError(null);
    } catch (e: any) {
      setError(e.message);
    } finally {
...
function useFirestoreDoc<T>(collection: string, id: string, seedData?: T) {
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      // Ensure doc exists + is seeded with defaults.
      // Important: the collection/doc may exist but be empty `{}` due to earlier seeding runs.
      // In that case, we overwrite with seedData so the admin UI has placeholders/fields to edit.
      if (seedData) {

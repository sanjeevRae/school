...
export default function AdminBlogs() {
  ...
  const handleSave = async (id: string) => {
    ...
    await update(id, {
      ...updates,
      order: typeof post?.order === "number" ? post.order : posts.findIndex((item) => item.id === id),
      updatedAt: new Date().toISOString(),
    });
    setEditing((prev) => {
      const next = { ...prev };
      ...
      <AdminLayout>
        ...
        {showNew && (
          <Card>
            <CardContent className="p-4 space-y-3">
              ...
              {newPost.image ? (
                <img src={newPost.image} alt={newPost.title || "Blog preview"} className="h-20 w-28 rounded object-cover border border-slate-200" />
              ) : null}
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {posts.map((post) => {
              const imageValue = editing[`${post.id}.image`] ?? post.image;
                ...
                <CardContent className="p-4 space-y-3">
                  ...
                  <div className="grid sm:grid-cols-3 gap-3">
                    <div>
                      ...
                      <Input defaultValue={post.author} onChange={(e) => handleEdit(post.id, "author", e.target.value)} placeholder="Author" />
                    </div>
                    <div>
                      <label className="text-xs text-slate-500">Category</label>
                      <Input defaultValue={post.category} onChange={(e) => handleEdit(post.id, "category", e.target.value)} placeholder="Category" />
                    </div>
                    <div>
                      <label className="text-xs text-slate-500">Published Date</label>
                      <Input type="date" defaultValue={post.publishedAt} onChange={(e) => handleEdit(post.id, "publishedAt", e.target.value)} />
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Textarea rows={8} defaultValue={post.content} onChange={(e) => handleEdit(post.id, "content", e.target.value)} placeholder="Write full blog content here" />
                  </div>
                  <div className="flex flex-wrap items-center gap-3">
                  ...
                  {imageValue ? (
                    <img src={imageValue} alt={post.title} className="h-20 w-28 rounded object-cover border border-slate-200" />
                  ) : null}
                  <div className="flex items-center justify-between">

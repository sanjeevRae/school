...
export default function AdminMedia() {
  ...
  const handleExistingImageUpload = async (id: string, file?: File) => {
    ...
    try {
      setUploadingId(id);
      const result = await uploadImage(file);
      const existingImages = editing[`${id}.images`] ?? images.find((img) => img.id === id)?.images ?? [];
  ...
  const handleRemoveExistingImage = (id: string, imageIndex: number) => {
    const currentImages = editing[`${id}.images`] ?? images.find((img) => img.id === id)?.images ?? [];
    handleEdit(
      id,
      "images",
            ...
            {images.map((img) => (
              <Card key={img.id}>
                <CardContent className="p-3 space-y-3">
                  <img src={(editing[`${img.id}.images`] ?? img.images)?.[0]} alt={img.title} className="w-full h-40 object-cover rounded-lg" />
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <Button type="button" variant="outline" size="sm" onClick={() => multiFileRef.current?.click()} disabled={uploading}>
                      <Plus className="h-4 w-4 mr-1" /> Change Picture
                    </Button>
                    {uploadingId === img.id && <span className="text-sm text-slate-500">Uploading...</span>}
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {(editing[`${img.id}.images`] ?? img.images).map((image: string, index: number) => (

...
export default function AdminMedia() {
  ...
  const showPrevPreview = (id: string, total: number) => {
  ...
  const showNextPreview = (id: string, total: number) => {
    setPreviewIndexes((prev) => ({
      ...prev,
        ...
        {loading ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {images.map((img) => {
              ...
              const activePreviewIndex = previewIndexes[img.id] ?? 0;
              const activePreviewImage = currentImages[activePreviewIndex] ?? currentImages[0];
                ...
                <CardContent className="p-3 space-y-3">
                  <div className="space-y-3">
                    <div className="relative overflow-hidden rounded-lg border border-slate-200 bg-slate-50">
                      <img src={activePreviewImage} alt={img.title} className="w-full h-40 object-cover" />
                      {currentImages.length > 1 && (
                        <>
                          <Button
                            ...
                            size="icon"
                            variant="outline"
                            className="absolute left-2 top-1/2 h-8 w-8 -translate-y-1/2 bg-white/90"
                            onClick={() => showPrevPreview(img.id, currentImages.length)}
                          >
                          ...
                          <Button
                            ...
                            size="icon"
                            variant="outline"
                            className="absolute right-2 top-1/2 h-8 w-8 -translate-y-1/2 bg-white/90"
                            onClick={() => showNextPreview(img.id, currentImages.length)}
                          >
                        ...
                        {currentImages.map((image: string, index: number) => (
                          <button
                            ...
                            className={`shrink-0 overflow-hidden rounded-md border ${
                              activePreviewIndex === index ? "border-blue-500" : "border-slate-200"
                            }`}
                          >
                            <img src={image} alt={`${img.title} thumbnail ${index + 1}`} className="h-14 w-20 object-cover" />
                          </button>
                        ))}
                      </div>
                    )}

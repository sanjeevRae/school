...
export default function AdminMedia() {
  ...
  const handleRemoveNewImage = (imageIndex: number) => {
    ...
    setNewPreviewIndex((prev) => {
      const nextLength = newImage.images.length - 1;
      if (nextLength <= 0) return 0;
      return Math.min(prev, nextLength - 1);
    });
  };

  const handleNextPreview = (id: string, total: number) => {
  ...
  const handlePrevPreview = (id: string, total: number) => {
    setPreviewIndexes((prev) => ({
      ...prev,
  ...
  const handleNextNewPreview = () => {
    setNewPreviewIndex((prev) => ((prev + 1) % newImage.images.length));
  };

  const handlePrevNewPreview = () => {
    setNewPreviewIndex((prev) => ((prev - 1 + newImage.images.length) % newImage.images.length));
  };
              ...
              {newImage.images.length > 0 && (
                <div className="space-y-3">
                  <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
                    <img src={newImage.images[newPreviewIndex]} alt={`Preview ${newPreviewIndex + 1}`} className="h-56 w-full object-cover" />
                    ...
                    {newImage.images.map((image, index) => (
                      <button
                        ...
                        className={`relative shrink-0 overflow-hidden rounded-lg border ${
                          newPreviewIndex === index ? "border-blue-500" : "border-slate-200"
                        }`}
                        onClick={() => setNewPreviewIndex(index)}
                      >
                        <img src={image} alt={`Preview ${index + 1}`} className="h-20 w-24 object-cover" />
                      </button>
                    ))}
                  </div>
                  <div className="flex gap-3 overflow-x-auto pb-1">
                    ...
                    <div key={`${image}-${index}`} className="relative shrink-0">
                      {index === newPreviewIndex ? null : null}
                      <Button
                        type="button"
                        size="icon"
                        variant="outline"
                        className="h-8 w-8 border-red-200 bg-white text-red-600 hover:bg-red-50 hover:text-red-700"
        ...
        {loading ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {images.map((img) => {
              ...
              const currentPreviewIndex = Math.min(previewIndexes[img.id] ?? 0, Math.max(currentImages.length - 1, 0));

              return (
              <Card key={img.id}>
                <CardContent className="p-3 space-y-3">
                  <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
                    <img src={currentImages[currentPreviewIndex]} alt={img.title} className="h-56 w-full object-cover" />
                    {currentImages.length > 1 && (
                      <>
                        <Button
                          ...
                          variant="secondary"
                          className="absolute left-2 top-1/2 h-9 w-9 -translate-y-1/2"
                          onClick={() => handlePrevPreview(img.id, currentImages.length)}
                        >
                          <ChevronLeft className="h-4 w-4" />
                        </Button>
                        <Button
                          ...
                          variant="secondary"
                          className="absolute right-2 top-1/2 h-9 w-9 -translate-y-1/2"
                          onClick={() => handleNextPreview(img.id, currentImages.length)}
                        >
                          <ChevronRight className="h-4 w-4" />
                        </Button>
                      </>
                  ...
                  <div className="flex gap-3 overflow-x-auto pb-1">
                    {currentImages.map((image: string, index: number) => (
                      <button
                        ...
                        type="button"
                        className={`relative shrink-0 overflow-hidden rounded-lg border ${
                          currentPreviewIndex === index ? "border-blue-500" : "border-slate-200"
                        }`}
                        onClick={() => setPreviewIndexes((prev) => ({ ...prev, [img.id]: index }))}
                      >
                        <img src={image} alt={`${img.title} ${index + 1}`} className="h-20 w-24 object-cover" />
                      </button>
                    ))}
                  </div>
                  <div className="flex gap-3 overflow-x-auto pb-1">
                    {currentImages.map((_: string, index: number) => (
                      <div key={`${img.id}-remove-${index}`} className="relative shrink-0">
                        <Button
                          type="button"
                          size="icon"
                          variant="outline"

              ...
              <div className="mt-4 grid grid-cols-4 sm:grid-cols-6 gap-2">
                {currentImages.map((image, index) => (
                  <button
                    ...
                    className={`overflow-hidden rounded border bg-slate-800 ${
                      lightboxImage === index ? "border-white" : "border-white/20"
                    }`}
                  >
                    <img src={image} alt={`${currentGroup.title} thumbnail ${index + 1}`} className="h-16 w-full object-contain bg-slate-800" />
                  </button>
                ))}
              </div>
            )}

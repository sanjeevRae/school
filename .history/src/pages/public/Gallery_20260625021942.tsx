              ...
              <div className="mt-4 grid grid-cols-4 sm:grid-cols-6 gap-2">
                {currentImages.map((image, index) => (
                  <button
                    ...
                    className={`flex h-20 items-center justify-center overflow-hidden rounded border bg-slate-800 p-1 ${
                      lightboxImage === index ? "border-white" : "border-white/20"
                    }`}
                  >
                    <img src={image} alt={`${currentGroup.title} thumbnail ${index + 1}`} className="max-h-full max-w-full object-contain" />
                  </button>
                ))}
              </div>
            )}

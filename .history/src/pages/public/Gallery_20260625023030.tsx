  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {lightboxGroup !== null && currentGroup && currentImages.length > 0 && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/95 px-4 py-6 sm:px-6 sm:py-8"
          onClick={closeLightbox}
        >
          <button
            onClick={closeLightbox}
            className="absolute right-4 top-4 rounded-full bg-slate-900/70 p-2 text-white/80 transition hover:text-white sm:right-6 sm:top-6"
          >
            <X className="h-8 w-8" />
          </button>
          <button
            ...
            className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-slate-900/70 p-2 text-white/80 transition hover:text-white sm:left-6"
          >
          ...
          <button
            ...
            className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-slate-900/70 p-2 text-white/80 transition hover:text-white sm:right-6"
          >
            <ChevronRight className="h-8 w-8" />
          </button>
          <div className="w-full max-w-5xl px-6 sm:px-12" onClick={(e) => e.stopPropagation()}>
            <div className="mx-auto flex max-h-[85vh] max-w-4xl flex-col gap-4 rounded-3xl bg-white/5 p-4 shadow-2xl backdrop-blur-sm sm:gap-5 sm:p-6">
              <div className="flex min-h-0 flex-1 items-center justify-center rounded-2xl bg-slate-950/60 p-4 sm:p-6">
                <img
              ...
              {currentImages.length > 1 && (
                <div className="rounded-2xl bg-slate-950/40 p-3 sm:p-4">
                    ...
                    {currentImages.map((image, index) => (
                      <button
                        ...
                        className={`overflow-hidden rounded-xl border p-1 transition-all ${
                          lightboxImage === index
                            ? "border-white bg-white/10 ring-2 ring-white/40"
                            : "border-white/10 bg-white/5 hover:border-white/40"
                        }`}
                      >
                        <div className="flex aspect-square w-full items-center justify-center rounded-lg bg-slate-900 p-2">
                          <img
                            src={image}

...
import { USE_MOCK_FIRESTORE } from "@/lib/firebase/config";
import { bootstrapWebsiteContent } from "@/lib/firebase/bootstrap";
...
function useFirestoreCollection<T>(collection: string, seedData?: T[]) {
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      let mapped = docs.map((d) => ({ ...d.data, id: d.id })) as T[];

      if (mapped.length === 0 && seedData) {
        mapped = seedData;
      }
...
function useFirestoreDoc<T>(collection: string, id: string, seedData?: T) {
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      if (!existing && seedData && !USE_MOCK_FIRESTORE) {
        await firestore.setDoc(collection, id, seedData as any, { merge: false });
      }

      const doc = existing ?? (USE_MOCK_FIRESTORE ? null : await firestore.getDoc(collection, id));
      if (doc) {

...
import { useAdmissionInfo, useSettings } from "@/hooks/useSchoolData";
...
export default function Admission() {
  ...
  const admission = {
    ...
    ...info,
    title: info?.title?.trim() || defaultAdmissionInfo.title,
    content: info?.content?.trim() || defaultAdmissionInfo.content,
    fees: info?.fees?.trim() || defaultAdmissionInfo.fees,
    process: Array.isArray(info?.process) && info.process.length ? info.process.filter(Boolean) : defaultAdmissionInfo.process,
    requirements:
      Array.isArray(info?.requirements) && info.requirements.length
        ? info.requirements.filter(Boolean)
        : defaultAdmissionInfo.requirements,
  };

  const contactPhone = settings?.phone?.trim() || "01-4970590";
  const contactEmail = settings?.email?.trim() || "info@samriddhischool.edu.np";
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-slate-800 py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">{admission.title}</h1>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2">
                ...
                {loading ? (
                  ...
                  <p className="text-slate-600 leading-relaxed mb-8">{admission.content}</p>
                )}
              ...
              <div className="bg-slate-50 rounded-xl p-6">
                ...
                <div className="mt-6 pt-6 border-t border-slate-200 space-y-4">
                  <div>
                    <h4 className="font-semibold text-slate-900 mb-2">Fees & Notes</h4>
                    <p className="text-sm text-slate-600 leading-relaxed">{admission.fees}</p>
                  </div>

                  <div>
                    <h4 className="font-semibold text-slate-900 mb-3">Contact Admissions</h4>
                    <div className="space-y-2 text-sm">
                      <a href={`tel:${contactPhone}`} className="flex items-center gap-2 text-slate-600 hover:text-amber-600">
                        <Phone className="h-4 w-4" /> {contactPhone}
                      </a>
                      <a href={`mailto:${contactEmail}`} className="flex items-center gap-2 text-slate-600 hover:text-amber-600">
